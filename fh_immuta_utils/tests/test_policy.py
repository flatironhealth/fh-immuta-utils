...
def data_policy_dict():
    return pol.PolicyConfig(config_root=f"{os.path.dirname(os.path.abspath(__file__))}/fixtures")
...
def test_make_policy_exceptions(data_policy_actions_dict):
    ...
    expected_exceptions = pol.PolicyExceptions(
        operator="and",
        conditions=[pol.GroupCondition(group=pol.PolicyGroup(name="group1", iam="okta"))]
    )
...
def test_make_policy_rule(data_policy_actions_dict, tagger):
    ...
    expected_rule = pol.PolicyRule(
        ...
        config=pol.MaskingRuleConfig(
            fields=[pol.ColumnTag(name=config_field_tags[0], hasLeafNodes=tagger.is_root_tag(config_field_tags[0]))],
...
def test_make_data_policy_action(data_policy_actions_dict, tagger):
    ...
    rule2_iam_groups = rule2_conditions[0]['iam_groups'] + rule2_conditions[1]['iam_groups']
    expected_action = pol.MaskingAction(
        ...
        rules=[
            pol.PolicyRule(
                ...
                config=pol.MaskingRuleConfig(
                    fields=[pol.ColumnTag(
                        name=rule1['config']['fields']['tags'][0],
                        hasLeafNodes=tagger.is_root_tag(rule1['config']['fields']['tags'][0]))],
                    maskingConfig=pol.MaskingConfig(type="Consistent Value"),
                ),
            ),
            pol.PolicyRule(
                ...
                config=pol.MaskingRuleConfig(
                    fields=[
                        pol.ColumnTag(
                            ...
                            hasLeafNodes=tagger.is_root_tag(rule2['config']['fields']['tags'][0])
                        ),
                        pol.ColumnTag(
                            name=rule2['config']['fields']['tags'][1],
                            hasLeafNodes=tagger.is_root_tag(rule2['config']['fields']['tags'][1])
                        ),
                    ],
                    maskingConfig=pol.MaskingConfig(type="Consistent Value"),
                ),
